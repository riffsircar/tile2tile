...
parser = argparse.ArgumentParser(description='Tile histogram evaluation')
...
if args.mod == 'ae':
    ...
    if args.game == 'met':
        if '{' not in hist_original:
            hist_original['{'] = 0
        if '}' not in hist_original:
            hist_original['}'] = 0
    if args.game == 'mm':
        if 'D' not in hist_original:
            hist_original['D'] = 0
    ...
    plt.bar(hist_original_norm_sorted.keys(), hist_original_norm_sorted.values(), color='brown')
    plt.ylabel('Occurrence Percentage', fontsize=16)
    plt.xlabel('Tiles', fontsize=16)
    plt.xticks(fontsize=16)
    plt.yticks(np.arange(0.0, 1.2, 0.2), fontsize=16)
    ...
    smb_mrf = pickle.load(open('trained_mrf/mrf_' + str(args.ns) + '_' + 'smb.pickle','rb'))
    ki_mrf = pickle.load(open('trained_mrf/mrf_' + str(args.ns) + '_' + 'ki.pickle','rb'))
    mm_mrf = pickle.load(open('trained_mrf/mrf_' + str(args.ns) + '_' + 'mm.pickle','rb'))
    met_mrf = pickle.load(open('trained_mrf/mrf_' + str(args.ns) + '_' + 'met.pickle','rb'))
    mrfs = {'smb':smb_mrf, 'ki':ki_mrf, 'mm':mm_mrf, 'met':met_mrf}
    hist_original, hist_generated = {}, {}
    ...
    plt.ylabel('Occurence Percentage', fontsize=18)
...
for from_game, hist_generated_from_game in hist_generated.items():
    ...
    if args.game == 'met':
        if '{' not in hist_generated_from_game:
            hist_generated_from_game['{'] = 0
        if '}' not in hist_generated_from_game:
            hist_generated_from_game['}'] = 0
    ...
    # print('HNS: ', hist_norm_sorted)
    x = np.arange(len(hist_norm_sorted))
    plt.bar(x+x_del, hist_norm_sorted.values(), bar_width, label=f'{from_game}-{args.game}'.upper())
    x_del += 0.3
    ...
    obs = np.array([hist_original_list,hist_gen_from_game_list])
    #print(obs.shape)
    chi, p, dof, freq = stats.chi2_contingency(obs, correction=True)
    print(f"p-value: {p}\tStatistic: {chi}")
...
plt.ylabel('Occurrence Percentage', fontsize=16)
plt.xlabel('Tiles', fontsize=16)
plt.xticks(np.arange(len(hist_norm_sorted))+x_del_width, hist_norm_sorted.keys(), fontsize=16)
plt.yticks(np.arange(0.0, 1.2, 0.2), fontsize=16)
if args.mod == 'ae':
